...
export const genGrothZKey = async (outputDir: string, circuitName: string, powerofTauPath: string): Promise<void> => {
    ...
    await zKey.contribute(zKeyPath_0000, zKeyPath_0001, "1st Contributor Name", "askljdaklsdjkls")
    await zKey.contribute(zKeyPath_0001, zKeyPath_0002, "2nd Contributor Name", "askljdaklsdjkls")

    await zKey.exportBellman(zKeyPath_0002, challenge_phase2_0003)
    const curve = await getCurveFromName("bn128")
    await zKey.bellmanContribute(curve, challenge_phase2_0003, response_phase2_0003, "asjdakdklasjdkl")
    await zKey.importBellman(zKeyPath_0002, response_phase2_0003, zKeyPath_0003, "third contribution name")
    ...
    await exec(`snarkjs zkey beacon ${zKeyPath_0003} ${zKeyFinal} 0102030405060708090a0b0c0d0e0f101112131415161718191a1b1c1d1e1f 10 -n="Final Beacon phase2"`)
    // if(stderr){
...
export const genPlonkZKey = async (cktr1csFp: string, ptauFp: string, zkeyFp: string): Promise<void> => {
    return plonk.setup(cktr1csFp, ptauFp, zkeyFp);
}

export const genVerificationKey = async (zKeyPath: string, vKeyPath: string): Promise<void> => {
